...
- Level progression is no longer capped at 50. At Lv50+ the optimizer switches to an explicit experiment mode: it keeps timing levels and mirroring normal upgrades, does not invent unobserved post-50 Level Up rewards, and only optimizes the current level because the relationship between extra levels and `Ingots gained` is not known yet.
- A Prestige-efficiency probe records `Ingots gained` at arbitrary Lv50+ checkpoints and compares marginal post-50 seconds per extra Ingot against the current run's exact Lv2→50 seconds per Ingot. These observations persist across Prestige resets and are included in JSON exports.
