...
- Cash tracking now defaults to AUTO: real Cash sync points are converted into net $/s samples (purchase spend is added back, level-transition intervals are excluded), noisy samples are summarized robustly by the median and interquartile range, and prior-run samples can be scaled to the current upgrade/permanent-power state when the current level has too little data. Manual $/s remains available as an override.
- The primary recommendation is time-aware rather than affordable-only ROI: a bounded 3-purchase lookahead estimates wait-to-afford from learned cash pace, carries the opportunity cost of waiting forward, and compares that against immediate cheaper purchases using the remaining-to-Lv50 timing curve learned from exact prior-run level timings and current-run pace.
