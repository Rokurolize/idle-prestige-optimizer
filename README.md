...
- Route-progress effects are separated from raw cash/DPS effects: the empirical timing model gives sublinear credit to DPS-like upgrades, full expected-value credit to Rare, smaller supply credit to Feed, and no direct EXP credit to Spike Size until an EXP effect is actually observed.
...
- At Lv50+, persistent normal upgrades are valued over a rolling multi-Ingot investment horizon rather than only the current level. The horizon is bounded to the next eight Ingots and shrinks automatically as the current marginal level time approaches the measured Lv2→50 reset cycle, while waits are still forbidden from crossing the current level's conservative EXP-full boundary.
