...
s=s.replace('<script>\n(() => {',seed+'<script>\n(() => {',1)
probe=r'''<script>setTimeout(()=>{document.getElementById('prestigeGainProbe').value='2';document.getElementById('recordPrestigeProbe').click();setTimeout(()=>{const text=document.getElementById('prestigeProbeVerdict').textContent,out=document.createElement('pre');out.id='plateau-result';out.textContent=JSON.stringify({text,pass:text.includes('Level − 49')&&text.includes('16分00秒/▲')});document.body.appendChild(out)},80)},100)</script>'''
