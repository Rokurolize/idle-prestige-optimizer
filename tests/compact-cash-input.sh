...
probe=r'''<script>setTimeout(()=>{const level=document.getElementById('level');level.value='22';level.dispatchEvent(new Event('change',{bubbles:true}));setTimeout(()=>{const cash=document.getElementById('cash');const gravityBefore=document.querySelector('.buy[data-key="gravity"]').disabled;cash.value='47.65M';cash.dispatchEvent(new Event('paste',{bubbles:true}));setTimeout(()=>{const displayed=cash.value;const gravityAfter=document.querySelector('.buy[data-key="gravity"]').disabled;const out=document.createElement('pre');out.id='compact-cash-result';out.textContent=JSON.stringify({displayed,gravityBefore,gravityAfter,pass:displayed==='47.65M'&&gravityBefore===false&&gravityAfter===false});document.body.appendChild(out)},80)},60)},120)</script>'''
