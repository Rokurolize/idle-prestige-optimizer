...
s=s.replace('<script>\n(() => {', seed+'<script>\n(() => {', 1)
probe=r'''<script>setTimeout(()=>{const dps=Number(document.getElementById('dps').value),exp=Number(document.getElementById('reducerExponent').value),rounded=(dps/1000).toFixed(2)+'K';const out=document.createElement('pre');out.id='reducer-exp-result';out.textContent=JSON.stringify({dps,rounded,exp,pass:rounded==='5.52K'&&Math.abs(exp-1.2711)<1e-9});document.body.appendChild(out)},160)</script>'''
