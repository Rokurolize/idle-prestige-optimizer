...
s=s.replace('<script>\n(() => {',seed+'<script>\n(() => {',1)
probe=r'''<script>setTimeout(()=>{const eta=document.getElementById('levelEta'),text=eta.textContent,title=eta.title,nums=(text.match(/\d+/g)||[]).map(Number),rec=document.getElementById('recommend').textContent,pass=text.includes('〜')&&nums.length>=2&&nums[0]<=18&&nums[1]>=25&&title.includes('中央予測')&&rec.includes('EXP満タンの早い側');const out=document.createElement('pre');out.id='eta-ensemble-result';out.textContent=JSON.stringify({text,title,rec:rec.slice(0,220),pass});document.body.appendChild(out)},250)</script>'''
