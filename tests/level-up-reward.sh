...
probe=r'''<script>setTimeout(()=>{const level=document.getElementById('level');const cash=document.getElementById('cash');const auto=document.getElementById('cashAuto');auto.checked=false;auto.dispatchEvent(new Event('change',{bubbles:true}));level.value='11';level.dispatchEvent(new Event('change',{bubbles:true}));cash.value='1000';cash.dispatchEvent(new Event('change',{bubbles:true}));setTimeout(()=>{document.getElementById('levelUp').click();setTimeout(()=>{const actual=JSON.parse(localStorage.getItem('prestige-route-optimizer-v1')).cash,expected=1471.03;const out=document.createElement('pre');out.id='lvup-result';out.textContent=JSON.stringify({level:document.getElementById('level').value,actual,expected,pass:document.getElementById('level').value==='12'&&Math.abs(actual-expected)<0.005});document.body.appendChild(out)},50)},50)},100)</script>'''
