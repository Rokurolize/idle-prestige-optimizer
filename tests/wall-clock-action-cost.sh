...
probe=r'''<script>setTimeout(()=>{const text=document.getElementById('recommend').textContent,out=document.createElement('pre');out.id='wall-clock-result';out.textContent=JSON.stringify({text,pass:text.includes('購入せず')&&text.includes('操作コスト')&&!document.getElementById('applyBest')});document.body.appendChild(out)},160)</script>'''
