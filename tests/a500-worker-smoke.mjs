...
try{
  ...
  assert.equal(output.modelRevision,'r82-runtime-20260831a');assert.ok(output.milliseconds<10000);assert.equal(output.legacyPending,true);assert.ok(output.result?.plan?.closedLoop);assert.equal(output.result.plan.legacyPending,true);assert.equal(output.result.plan.campaign.allowLegacy,false);assert.ok(Number.isFinite(output.result.plan.totalOverlapSeconds));assert.deepEqual(bad,[],'worker preview must not contain NaN/Infinity/undefined');
  console.log(JSON.stringify({milliseconds:output.milliseconds,totalSeconds:output.result.plan.totalOverlapSeconds,edges:output.result.plan.campaign.edgeEvaluations,legacyPending:output.legacyPending},null,2));
