...
s=s.replace('<script>\n(() => {', seed+'<script>\n(() => {', 1)
probe=r'''<script>setTimeout(()=>{const dps=Number(document.getElementById('dps').value),spikeCost=Number(document.querySelector('.cost[data-key="spikeCount"]').value),feedCost=Number(document.querySelector('.cost[data-key="feed"]').value),feedGrowth=Number(document.querySelector('.growth[data-key="feed"]').value);const out=document.createElement('pre');out.id='migration-result';out.textContent=JSON.stringify({dps,spikeCost,feedCost,feedGrowth,pass:Math.abs(dps-320.78)<.2&&spikeCost===1200&&feedCost===400&&Math.abs(feedGrowth-1.4)<1e-9});document.body.appendChild(out)},150)</script>'''
