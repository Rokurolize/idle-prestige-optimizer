...
s=s.replace('<script>\n(() => {',seed+'<script>\n(() => {',1)
probe=r'''<script>setTimeout(()=>{const text=document.getElementById('recommend').textContent,out=document.createElement('pre');out.id='wait-plan-result';out.textContent=JSON.stringify({text,pass:text.includes('Power')&&text.includes('待')&&text.includes('時間ベース3手先読み')});document.body.appendChild(out)},250)</script>'''
